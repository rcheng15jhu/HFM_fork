...
if __name__ == "__main__":
    ...
    # load model
    # sess = tf.Session()
    # saver = tf.train.import_meta_graph(safe_path + f'{save_name}_model.ckpt.meta')
    # saver.restore(sess, tf.train.latest_checkpoint(safe_path))
    # model = HFM(t_data, x_data, y_data, z_data, u_data, v_data, w_data,
    #             # t_eqns, x_eqns, y_eqns, z_eqns,
    #             layers, batch_size,
    #             Rey)
    model.saver = tf.train.import_meta_graph(safe_path + f'{save_name}_model.ckpt.meta')
    model.saver.restore(model.sess, model.save_path)
    ...
    scipy.io.savemat( safe_path + f"Vel3D_{save_name}_continue_results_{time.strftime('%d_%m_%Y')}.mat",
